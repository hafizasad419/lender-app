import { Users, Building2, ClipboardList, Banknote, CalendarDays, Coins, TimerReset, FileText,Tag } from "lucide-react";
import { formatDebtType, obfuscateName } from "@src/utils"

const PortfolioInfo = ({ portfolio }: { portfolio: any }) => {
    const details = [
        { label: "Lender Name", value:obfuscateName(portfolio?.lenderName) , icon: <Users className="w-5 h-5 mr-2 text-zinc-500" /> },
        { label: "Organization", value: obfuscateName(portfolio?.lenderOrganization), icon: <Building2 className="w-5 h-5 mr-2 text-zinc-500" /> },
        ...
        { label: "Debt type", value: formatDebtType(portfolio?.debtType), icon: <Tag className="w-5 h-5 mr-2 text-zinc-500" /> },
    ];
